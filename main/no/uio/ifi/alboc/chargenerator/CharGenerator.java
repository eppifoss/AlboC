...
public class CharGenerator {
	...
	if(curC == '/' && nextC == '*'){
	    int commentStart = curLineNum();
	    		
	    while (! (curC == '*' && nextC == '/')){
		readNext();
		if( (curC == '/') && (nextC == '*')){
		    Error.error(commentStart," cannot have comment inside a comment!");
		    Log.noteError("Has comment inside a comment in line "+ commentStart+" !");
		}
